...
logger = logging.getLogger('2000_charge')
...
if not (json and hasattr(json, 'loads')):
    try:
        ...
        if not json:
            raise ImportError(
                "2000Charge requires a JSON library, such as simplejson. "
                "HINT: Try installing the "
                "python simplejson library via 'pip install simplejson' or "
                "'easy_install simplejson'.")
        else:
            raise ImportError(
                "2000Charge requires a JSON library with the same interface as "
